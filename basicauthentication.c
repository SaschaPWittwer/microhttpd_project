...
static int userRequestPOST(void *cls, struct MHD_Connection *connection, const char *url, const char *method, const char *upload_data, size_t *upload_data_size, void **con_cls)
{
...
static int userRequestGET(void *cls, struct MHD_Connection *connection, const char *url, const char *method, const char *upload_data, size_t *upload_data_size, void **con_cls)
{
  int fail;
...
//Dispatching requests to different Routes
static int requestDispatcher(void *cls, struct MHD_Connection *connection, const char *url, const char *method, const char *version, const char *upload_data, size_t *upload_data_size, void **con_cls)
{
  ...
  if (0 == strcmp(url, "/user"))
  {
    ...
    {
      ret = userRequestPOST(cls, connection, url, method, upload_data, upload_data_size, con_cls);
    }
    if (0 == strcmp(method, MHD_HTTP_METHOD_GET))
    {
      ret = userRequestGET(cls, connection, url, method, upload_data, upload_data_size, con_cls);
    }
  }
  else if (0 == strcmp(url, "/other"))
  {
    return MHD_NO;
  }
  else
  {
    if ((0 != strcmp(method, MHD_HTTP_METHOD_GET)) && (0 != strcmp(method, MHD_HTTP_METHOD_POST)) && (0 != strcmp(method, MHD_HTTP_METHOD_PUT)))
    {
      return MHD_NO;
      json_t *j = json_pack("{s:i}", "ERROR", 5);
      char *s = json_dumps(j, 0);
      ret = send_json(connection, s, MHD_HTTP_INTERNAL_SERVER_ERROR);
    }
  }
...
int main()
{
  ...
  daemon = MHD_start_daemon(MHD_USE_INTERNAL_POLLING_THREAD, PORT, NULL, NULL, &requestDispatcher, NULL, MHD_OPTION_END);
